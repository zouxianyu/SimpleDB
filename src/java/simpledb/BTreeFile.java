...
public class BTreeFile implements DbFile {
    ...
    private BTreeLeafPage findLeafPage(TransactionId tid, HashMap<PageId, Page> dirtypages, BTreePageId pid, Permissions perm,
        ...
        // now, the current page must be a internal page
    ...
    protected void stealFromLeafPage(BTreeLeafPage page, BTreeLeafPage sibling,
        ...
        if (isRightSibling) {
            ...
            while (page.getNumTuples() < (page.getNumTuples() + sibling.getNumTuples()) / 2) {
                Tuple stolenTuple = rightIterator.next();
                sibling.deleteTuple(stolenTuple);
                page.insertTuple(stolenTuple);
            }
            // make sure the parent entry equals to the key of the first tuple in the right page
            entry.setKey(rightIterator.next().getField(keyField()));
        } else {
            // steal from the left sibling
            Iterator<Tuple> leftIterator = sibling.reverseIterator();
            while (page.getNumTuples() < (page.getNumTuples() + sibling.getNumTuples()) / 2) {
                Tuple stolenTuple = leftIterator.next();
                sibling.deleteTuple(stolenTuple);
                page.insertTuple(stolenTuple);
            }
            // make sure the parent entry equals to the key of the first tuple in the right page
            // the stolen tuple is the last tuple in the left page before,
            // so it is the first tuple in the right page now
    ...
    protected void stealFromLeftInternalPage(TransactionId tid, HashMap<PageId, Page> dirtypages,
        ...
        while (page.getNumEntries() < (leftSibling.getNumEntries() + page.getNumEntries()) / 2) {
            // get left entry and right entry
            BTreeEntry leftEntry = leftSibling.reverseIterator().next();
            BTreeEntry rightEntry = page.iterator().next();

            Field parentKey = leftEntry.getKey();

            // move the right-most entry from the left page to the right page
    ...
    protected void stealFromRightInternalPage(TransactionId tid, HashMap<PageId, Page> dirtypages,
        ...
        while (page.getNumEntries() < (page.getNumEntries() + rightSibling.getNumEntries()) / 2) {
            ...
            // move the left-most entry from the right page to the left page
    ...
    protected void mergeLeafPages(TransactionId tid, HashMap<PageId, Page> dirtypages,
        ...
        while (rightIterator.hasNext()) {
            ...
            rightPage.deleteTuple(rightTuple);
            leftPage.insertTuple(rightTuple);
        }

        // update the sibling pointers (skip the right page)
    ...
    protected void mergeInternalPages(TransactionId tid, HashMap<PageId, Page> dirtypages,
        ...
        // copy the parent entry to the left page
